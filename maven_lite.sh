...
function help()
{
    ...
    echo ""
    echo "  -cr, --create         Créer l'arborescence du projet ainsi qu'un"
...
function createProjet
{
    ...
    if [ -d $projectFolder ]
    then
        read -p "Le dossier '$projectFolder' existe déjà. Voulez-vous le supprimer ? (y/n) : " reponse
        [[ ${reponse} =~ ^[yY]([eE][sS])?$ ]] && { rm -rf $projectFolder || { echo "Erreur lors de la suppression du dossier '$projectFolder'"; exit 1; } } || { echo "Le dossier '$projectFolder' n'a pas été supprimé"; exit 0; }
    else
        mkdir -p $projectFolder || { echo "Erreur lors de la création du dossier '$projectFolder'"; exit 1; }
    fi

    echo "Création de l'arborescence du projet..."
    mkdir -p "$1/src" || { echo "Erreur lors de la création du dossier 'src'"; exit 1; }
    mkdir -p "$1/bin" || { echo "Erreur lors de la création du dossier 'bin'"; exit 1; }
    mkdir -p "$1/data" || { echo "Erreur lors de la création du dossier 'data'"; exit 1; }
...
do
    case "${ancienArg}" in
        ...
        "-cr") createProjet $arg && exit 0 || exit 1 ;;
        "--create") createProjet $arg && exit 0 || exit 1 ;;
...
if [ $compilation -eq 1 ] && [ $lancement -eq 1 ]
then
    help 1 "Aucune action donnée"
fi
